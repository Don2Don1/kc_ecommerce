...
def product_category(request, slug):
    prod_cat = Product.objects.filter(category__slug=slug).all()
    context = {
...
def signin(request):
    if request.method == "POST":
        ...
        if user is not None:
            login(request, user)
            messages.success(request, f"welcome {username}!! Flickbusters, we hope u have the best experince ")
            return redirect("index")
        else:
            messages.error(request,"Invalid username or password. Please condirm again")
...
def password(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request, "Password Chaged Succesfully")
            return redirect("profile")
        else: 
...
def search(request):
    if request.method == "POST":
        ...
        context = {
            "items": items,
            "searched__goods": searched_goods,
        }
...
@login_required(login_url="signip")
def modifycart(request):
    try:
        if request.method == "POST":
            quantity = int(request.POST["quantity"])
            items = request.POST["prodcut_id"]
...
def checkout (request):
    ...
    # sk_test_69e04d89183c4bc9fa84d057f8d83680c0ff9f6b
import json
@login_required(login_url="signin")
def pay(request):
    if request.method == "POST":
        api_key = "sk_test_69e04d89183c4bc9fa84d057f8d83680c0ff9f6b"
        curl = "https://api.paystack.co/transaction/initialize"
        cburl ="https://kc-ecommerce-fcee.onrender.com/callback/"   
        # cburl ="http://127.0.0.1:8992/callback/"   
        ref = str(uuid.uuid4())
        profile = Profile.objects.get(user__username= request.user.username)
        ...
        try:
            ...
            messages.error(request, "Network Busy, Try agai later....")
        else:
